...
public class MultiMovementTracker {
    ...
    private final List<MultiMovementTracker.TrackedRecognition> trackedObjects = new LinkedList<MultiMovementTracker.TrackedRecognition>();
    ...
    public MultiMovementTracker(final Context context) {
    ...
    public synchronized void draw(final Canvas canvas) {
        ...
        frameToCanvasMatrix =
                ImageUtils.getTransformationMatrix(
                        ...
                        sensorOrientation,
                        false);
        for (final MultiMovementTracker.TrackedRecognition recognition : trackedObjects) {
    ...
    public synchronized void onFrame(
        ...
        final LinkedList<MultiMovementTracker.TrackedRecognition> copyList =
                new LinkedList<MultiMovementTracker.TrackedRecognition>(trackedObjects);
        for (final MultiMovementTracker.TrackedRecognition recognition : copyList) {
            final ObjectTracker.TrackedObject trackedObject = recognition.trackedObject;
    ...
    private void processResults(
        ...
        if (objectTracker == null) {
            ...
            for (final Pair<Float, Classifier.Recognition> potential : rectsToTrack) {
                final MultiMovementTracker.TrackedRecognition trackedRecognition = new MultiMovementTracker.TrackedRecognition();
    ...
    private void handleDetection(
        ...
        final List<MultiMovementTracker.TrackedRecognition> removeList = new LinkedList<MultiMovementTracker.TrackedRecognition>();
        ...
        MultiMovementTracker.TrackedRecognition recogToReplace = null;
        ...
        for (final MultiMovementTracker.TrackedRecognition trackedRecognition : trackedObjects) {
        ...
        if (availableColors.isEmpty() && removeList.isEmpty()) {
            for (final MultiMovementTracker.TrackedRecognition candidate : trackedObjects) {
        ...
        for (final MultiMovementTracker.TrackedRecognition trackedRecognition : removeList) {
            logger.v(
    ...
    public void addNewResult(Classifier.Recognition recognition){
        if(trackerMap.containsKey(recognition.getTitle())){
            ...
            arrayList.add(box);
            logger.d("add %s box location : %s",recognition.getTitle(), box);
    ...
    public void drawMotionLines(Canvas canvas){
        int i = 0;
